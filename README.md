...
确认草案后，详细规划直接使用该版本的每日地点、顺序和住宿，不再让模型重新选路线，也不因驾驶超限自动删点。约束冲突会明确提示，调整沿用“修改预览 → 用户确认”流程；草案天数或终点与需求不一致时，需先更新草案再确认。确认按钮携带草案版本，旧版本确认会被拦截。行程保存确认草案快照及来源版本（旧行程不补造来源）；公开分享仅保留版本号，不包含私人草案备注。
